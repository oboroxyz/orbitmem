...
export function createPersistenceLayer(config: PersistenceConfig): IPersistenceLayer & {
  archive(opts?: { data?: Uint8Array; entryCount?: number; label?: string; pinToFilecoin?: boolean }): Promise<Snapshot>;
} {
  ...
  function generateCID(): string {
    ...
    return 'bafy' + Array.from(bytes).map(b => b.toString(36)).join('').slice(0, 55);
  }
