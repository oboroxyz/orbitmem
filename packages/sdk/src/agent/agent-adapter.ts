...
export function createOrbitMemAgentAdapter(config: AgentAdapterConfig): IOrbitMemAgentAdapter {
  const transport = createTransportLayer({
    signer: async (payload) => ({
      signature: await config.wallet.signMessage(payload),
      algorithm: config.wallet.family === 'evm' ? 'ecdsa-secp256k1'
        : config.wallet.family === 'solana' ? 'ed25519' : 'p256',
    }),
